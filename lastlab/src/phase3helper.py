...
SCHMOVE_TOLERANCE = 0.0375
MAX_ANGSPEED = 0.56 # 0.5 worked wellish lil slow
MAX_LINSPEAD = 0.105
...
class Phelper:
    ...
    # WE HAVE TROUBLES MOVING DIRECTLY DOWNWARDS
    def schmoveTo(self, targetPos: Point) :
        ...
        while not self.stop and distError > SCHMOVE_TOLERANCE:
            ...
            distFromStart = math.dist((self.px, self.py), (startX, startY))

            print("DE", round(distError, 4), "LS: ", round(linear_speed,4), "AS: ", round(ang_speed,4), "LSP: ", round(lin_speedPenalty,4))
            self.send_speed(linear_speed, ang_speed)
            # print("Drive error: ", round(error, 3))
            rospy.sleep(0.01)
    ...
    # TODO, I WOULD love an implementation OF PID based drive to position. 
